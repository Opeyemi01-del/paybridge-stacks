...
import { getMerchantOnChain, registerMerchantOnChain } from "../src/services/stacksService.js";
...
merchantsRouter.post("/register", async (req: Request, res: Response) => {
  try {
    ...
    if (!name || !webhook_url || !private_key) {
      res.status(400).json({
        error: "Missing required fields: name, webhook_url, private_key"
      });
      return;
    }
    ...
    const txResult = await registerMerchantOnChain(
      merchantId, name, webhook_url, private_key
    );
    ...
    res.status(201).json({
      ...
      message:     "Merchant registered. Transaction broadcast to Stacks network.",
    });
  } catch (err: any) {
